...
def get_deployment_parser(ForceArgument=False, Command="deploy"):
    ...
    parser.add_argument('-g', '--generated-lambda-layer', required=False, action='store_true', help='[optional] Forces rdk deploy to use the Python(3.6-lib,3.7-lib,3.8-lib,) lambda layer generated by rdk init --lambda-layer')
    ...
    def deploy(self):
        ...
        if self.args.generated_lambda_layer:
            ...
            if not lambda_layer_version:
                print(f"--generated-lambda-layer flag received, but rdk-lib-layer not found in {my_session.region_name}. Creating one now")
                lambda_layer_version = self.__create_new_lambda_layer(my_session)
        else:
            lambda_layer_version = None
