...
def get_init_parser():
    ...
    parser.add_argument('--custom-layer-name', required=False, help='[optional] Sets the name of the generated lambda-layer, "rdklib-layer" by default')
...
class rdk:
    ...
    def __create_new_lambda_layer(self, session, layer_name="rdklib-layer"):
        ...
        if layer_name == "rdklib-layer":
            return_code = self.__create_new_lambda_layer_serverless_repo(session)

        # If that doesn't work, create it locally and upload - SAR doesn't support the custom layer name
        if layer_name != "rdklib-layer" or not return_code:
            if not return_code:
    ...
    def __create_new_lambda_layer_serverless_repo(self, session):
        try:
            ...
            # Try to create a change set
            except ClientError as ce:
                if ce.response['Error']['Code'] == "ValidationError":
                    cfn_stack_url = sar_client.create_cloud_formation_template(ApplicationId=RDKLIB_LAYER_SAR_ID)["TemplateUrl"]
                    print(f"[{session.region_name}]: Creating new stack to deploy rdklib-layer")
                    cfn_client.create_stack(StackName='serverlessrepo-rdklib',TemplateURL=cfn_stack_url)
                    waiter = cfn_client.get_waiter('stack_create_complete')
                    waiter.wait(StackName='serverlessrepo-rdklib')
                    print(f"[{session.region_name}]: Successfully deployed")
