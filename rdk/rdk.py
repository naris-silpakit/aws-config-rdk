...
def get_deployment_parser(ForceArgument=False, Command="deploy"):
    ...
    parser.add_argument('-g', '--generated-lambda-layer', required=False, action='store_true', help='[optional] Forces rdk deploy to use the Python lambda layer generated by rdk init --lambda-layer')
...
class rdk:
    ...
    def deploy(self):
        ...
        if self.args.generated_lambda_layer:
            self.args.rdklib_layer_arn = self.__get_existing_lambda_layer(my_session)
            print (f"Found existing rdklib layer: {self.args.rdklib_layer_arn}")
